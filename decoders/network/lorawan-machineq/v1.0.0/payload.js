...
if (machineq_payload) {
  const serie = String(machineq_payload.serie || new Date().getTime()); // Get a unique serie for the incoming data.
  ...
  // fix so that the connectors work since most of them dont look for the key from "payload_hex"
  if (machineq_payload.payload_hex) {
    machineq_payload.payload = machineq_payload.payload_hex;
  }
