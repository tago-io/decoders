...
const ignore_vars = ['EUI', 'ts', 'freq', 'dr', 'ack'];
...
function toTagoFormat(object_item, serie, prefix = '') {
  ...
  for (const key in object_item) {
    if (ignore_vars.includes(key) || !object_item[key]) continue; // ignore chosen vars
...
if (loriot_payload) {
  const serie = String(loriot_payload.serie || new Date().getTime()); // Get a unique serie for the incoming data.

  // Parse the loriot_payload to JSON format (it comes in a String format)
  loriot_payload = JSON.parse(loriot_payload.value);

  if (loriot_payload.cmd === 'gw') {
    payload = [];
  } else {
    let vars_to_tago = [];
