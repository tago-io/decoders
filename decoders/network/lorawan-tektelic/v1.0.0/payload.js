...
const ignore_vars = ['rf_chain', 'channel', 'modulation', 'app_id', 'dev_id', 'time', 'gtw_trusted', 'port', "1_name", "id_id", "1_mac", 'rxinfo_time', 'rxinfo_rfchain', 'datarate_modulation', 'datarate_bandwidth', 'rxinfo_coderate', 'datarate_spreadfactor', 'rxinfo_mac', 'rxinfo_antenna', 'rxinfo_timestamp', 'adr'];
...
function toTagoFormat(object_item, serie, prefix = '') {
  ...
  for (const key in object_item) {
    ...
    if (typeof object_item[key] === 'object') {
      result.push({
        variable: (object_item[key].variable || `${prefix}${key}`).replace(/ /g, "_"),
      ...
      result.push({
        variable: `${prefix}${key}`.replace(/ /g, "_"),
...
function inspectFormat(object_item, serie, old_key) {
  ...
  for (const key in object_item) {
    if (key === 'lng' || key === 'longitude') continue;
      ...
      result.push({
        variable: (old_key ? `${old_key}_${key}`.toLowerCase() : `${key}`.toLowerCase()).replace(/ /g, "_"),
        value: object_item[key],
        serie,
      });
    }
...
if (tektelic_payload) {
  ...
  const serie = String(tektelic_payload.serie || new Date().getTime());
  ...
  if (tektelic_payload.payload) {
    ...
    if (tektelic_payload.payload.bytes) {
      tektelic_payload.payload.payload = tektelic_payload.payload.bytes;
      delete tektelic_payload.payload.bytes;
    }

    if (tektelic_payload.payload && tektelic_payload.payload.payload && tektelic_payload.payload.payload.includes('[')) {
      tektelic_payload.payload.payload = Buffer.from(JSON.parse(tektelic_payload.payload.payload)).toString('hex');
    }
