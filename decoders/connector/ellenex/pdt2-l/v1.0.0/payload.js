...
function parsePayload(payload_raw) {
  try {
    ...
    if (sensor_reading_type === 0) {
      const differential_pressure = bytes.readIntBE(3, 2);
      // differential_pressure = sensor output
      const temperature = bytes.readUIntBE(5, 2) / 100;
      const battery = bytes.readUIntBE(7, 1) * 0.1;

      data.push({ variable: 'differential_pressure', value: differential_pressure, unit: 'Pascal' },
