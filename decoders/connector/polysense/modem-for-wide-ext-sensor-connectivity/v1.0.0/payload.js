...
const parseDataType14 = x => ({ value: Number((x.readInt16BE() * 0.1).toFixed(1)), unit: '°C', metadata: { hex: x.toString('hex') } }); // Temperature data from external temperature sensor
  ...
  while (i < payload_raw.length) {
    switch (payload_raw[i]) {
      ...
      case 14:
        ...
        i += 3;
        if (i >= payload_raw.length) {
          insertData(decoded, 'external_temperature_sensor0', external_temperatures[0]);
        }
        break;
      case 15:
...
if (payload_raw) {
  ...
  if (value) {
    payload = payload.concat(toTagoFormat(parsePayload(Buffer.from(payload_raw.value, 'hex')), serie));
  }
  // console.log(payload);
}
