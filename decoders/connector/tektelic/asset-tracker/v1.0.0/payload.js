    ...
    decoder = [
      {
        key: [0x00, 0xBA],
        fn(arg) {
          decoded_data.battery_status_life = 2.5 + decode_field(arg, 0, 6, 'unsigned') * 0.01;
...
if (payload_raw) {
  ...
  if (value) {
    payload = payload.concat(toTagoFormat(Decoder(Buffer.from(value.replace(/ /g, ''), 'hex'), Number(port.value)), serie));
  }
}
