...
function toTagoFormat(object_item, serie, prefix = "") {
  ...
  for (const key in object_item) {
    ...
    if (object_item[key] === undefined || object_item[key] === null) {
      continue;
    }
    ...
    decoder = [
      {
        key: [0x00, 0xd3],
        fn(arg) {
          decoded_data.battery_status_life = { value: decode_field(arg, 0, 6, "unsigned"), "unit": "%" };
          return 1;
        },
      },
      {
        key: [0x00, 0xba],
        fn(arg) {
          decoded_data.battery_status_life = { value: battery_offset + decode_field(arg, 0, 6, "unsigned") * 0.01, unit: "V" };
      ...
      },
      {
        ...
        fn(arg) {
          ...
          for (let i = 0; i < arg.length * 8; i += 4 * 8) {
            dev_id = decode_field(arg, i, i + 3 * 8 - 1, "hexstring");
            if (beacon_decoder === "simple") {
              dev_id = `${dev_id}_beacon`;
            }
...
if (payload_raw) {
  ...
  if (value) {
    ...
    if (beacons.length) {
      decoded.beacons = {
        variable: "beacons",
        value: beacons.map((x) => `${x.replace("_beacon", "")}`).join("; "),
