...
const file_path = "decoders/connector/lansitec/socket-sync-proximity-gateway/v1.0.0/payload.ts" as const;
...
describe("Socket Sync Proximity Gateway heartbeat unit tests", () => {
  const payloadHex = "21CB3E033E006C00";
  const result = preparePayload(payloadHex);

  test("Check variable values", () => {
    expect(result.type?.value).toBe("Heartbeat");
    expect(result.battery?.value).toBe("3.53V");
