...
function bandToFrequency(band_str, device_time) {
  ...
  if (frequencyTable.hasOwnProperty(band))
    return ({ variable: "network_frequency", value: frequencyTable[band], unit: "MHz", time: device_time });
  return;
}
...
function parsePayload(payload) {
  try {
    ...
    data.push({ variable: 'time_last_input_br', value: dayjs(device_its.bt * 1000).tz("America/Sao_Paulo").format('HH:mm:ss DD/MM/YY') });
    ...
    if (net_info) {
      ...
      if (net_info.hasOwnProperty('operator'))
        data.push({ variable: 'operator', value: net_info.operator, time: device_time });
    }
