...
function parseAnalog(payload) {
  ...
  if (uplinkId.toUpperCase() === '0D') {
    let sensorName = payload.substring(2, 4);
...
function parseAnalogMeasurement(payload) {
  ...
  let k = 0;
  for (let i = 0; i < 4; i++) {
    const firstByte = parseInt(payload.substring(k + 8, k + 8 + 2), 16);
    const secondByte = parseInt(payload.substring(k + 8 + 2, k + 8 + 4), 16);

    const hexInBin = (((secondByte & 0xFF) << 8) & 0xFF00) + ((firstByte & 0xFF)) & 0x0000FFFF;
...
function parseDate(payload) {
  ...
  const binary = Number(parseInt(reverseBytes(payload), 16)).toString(2).padStart(32, '0');
...
// let payload = [{ variable: 'payload', value: '0D0180635729FF2FA426FF2FA126' }];
