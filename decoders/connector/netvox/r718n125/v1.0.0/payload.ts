function Decoder_r718n125(input, port) {
  switch (port) {
    case 6: // 0x06
...
function decodePort6_r718n125(bytes) {
  const data = [] as any;
  const cmd = bytes[2];
  ...
  switch (cmd) {
    case 0x00:
      ...
      data.push({
        variable: "date_code",
        value: (Number(bytes[5]) << 24) + (Number(bytes[6]) << 16) + (Number(bytes[7]) << 8) + Number(bytes[8])
      });
      return data;
    case 0x01:
      ...
      data.push({variable: "lowCurrent", value: (bytes[7] & 1) > 0 ? 'alarm' : 'noalarm'});
      data.push({variable: "highCurrent", value: (bytes[7] >> 1 & 1) > 0 ? 'alarm' : 'noalarm'});
      return data;
  }
}

function decodePort7_r718n125(bytes) {
  const data = [] as any;
  const cmd = bytes[0];
...
const payload_r718n125 = payload.find((x) => x.variable === "payload_raw" || x.variable === "payload" || x.variable === "data");
const port_r718n125 = payload.find((x) => x.variable === "port" || x.variable === "fPort" || x.variable === "fport")?.value;

if (payload_r718n125) {
  try {
    const buffer = Buffer.from(payload_r718n125.value, "hex");
    const data = Decoder_r718n125(buffer, port_r718n125);
    payload = payload.concat(data.map((x) => ({...x, serie: payload_r718n125.group, time: payload_r718n125.time})));
  } catch (error: any) {
    console.error(error);
