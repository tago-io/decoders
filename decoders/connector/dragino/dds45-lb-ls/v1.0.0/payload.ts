...
/**
 * Dragino DDS45-LB/LS - TagoIO Decoder
...
function datalogEntry(i: number, bytes: Buffer): string {
  const bat_raw = (bytes[0 + i] << 8) | bytes[1 + i];
  const distance = (bytes[2 + i] << 8) | bytes[3 + i];
  const temp = ((bytes[4 + i] << 8) | bytes[5 + i]) / 10;
  const flag1 = bytes[6 + i] & 0x01 ? "YES" : "NO";
  const flag2 = (bytes[6 + i] >> 1) & 0x01 ? "TRUE" : "FALSE";
  ...
  return `[${bat_raw},${distance},${temp},${flag1},${flag2},${flag3},${time}],`;
}
...
function Decoder(bytes: Buffer, port: number): Data[] | null {
  ...
  if (port === 5 && bytes.length === 7) {
    let sensor_model;
    if (bytes[0] === 0x28) sensor_model = "DDS45-LB";
