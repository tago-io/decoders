...
function datalog(i, bytes) {
  var aa = parseFloat(((bytes[2 + i] << 8) | bytes[2 + i + 1]) / 1000).toFixed(
    3,
  );
  var string = "[" + aa + "]";
  return string;
...
function transform(IDC_intput_mA, bytes2, bytes3) {
  ...
  if (bytes2 === 0x00) {
    if (IDC_intput_mA <= 4.0) idc_input_convert = 0;
    else
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * ((bytes3 * 100) / 16)).toFixed(3),
      );
  } else if (bytes2 == 0x01) {
    if (IDC_intput_mA <= 4.0) idc_input_convert = 0;
    else if (bytes3 == 1)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 0.0375).toFixed(3),
      );
    else if (bytes3 == 2)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 0.0625).toFixed(3),
      );
    else if (bytes3 == 3)
      idc_input_convert = parseFloat(((IDC_intput_mA - 4.0) * 0.1).toFixed(3));
    else if (bytes3 == 4)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 0.15625).toFixed(3),
      );
    else if (bytes3 == 5)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 0.625).toFixed(3),
      );
    else if (bytes3 == 6)
      ...
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * -0.00625).toFixed(3),
      );
    else if (bytes3 == 9)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 12.0) * 0.0125).toFixed(3),
      );
    else if (bytes3 == 10)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 0.3125).toFixed(3),
      );
    else if (bytes3 == 11)
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 3.125).toFixed(3),
      );
    else if (bytes3 == 12)
    ...
    if (IDC_intput_mA <= 4.0) idc_input_convert = 0;
      ...
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 18.75).toFixed(3),
      );
    else if (bytes3 == 4)
    ...
    if (IDC_intput_mA <= 4.0) idc_input_convert = 0;
      ...
      idc_input_convert = parseFloat(
        ((IDC_intput_mA - 4.0) * 1.875).toFixed(3),
      );
  }
...
function Decoder(bytes, port) {
  switch (port) {
    case 5:
      {
        if (bytes[0] == 0x16) data.SENSOR_MODEL = "Airflow-LN";
    ...
    default:
      ...
      if (bytes[2] === 0x00) {
        ...
        data.Differential_pressure_Pa = transform(
          IDC_intput_mA,
          bytes[2],
          bytes[3],
        );
      } else if (bytes[2] == 0x03) {
        data.wind_speed = transform(IDC_intput_mA, bytes[2], bytes[3]);
      }
      break;
  }
}
...
// Filter out ignore_vars for when we didn't run toTagoFormat on line 97.
payload = payload.filter((obj) => !ignore_vars.includes(obj.variable));
