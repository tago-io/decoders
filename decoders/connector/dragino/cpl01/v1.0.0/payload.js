...
function Decoder(bytes, port) {
  if (port === 0x02) {
    ...
    const pin_status = bytes[0] & 0x01 ? "DISCONNECT" : "CONNECT";
    const calculate_flag = (bytes[0] & 0xfc) >> 2;
    const total_pulse = (bytes[1] << 16) | (bytes[2] << 8) | bytes[3];
    const disconnect_duration = (bytes[4] << 16) | (bytes[5] << 8) | bytes[6];
    ...
    if (bytes.length === 11) {
      return {
        ...
        LAST_DISCONNECT_DURATION: disconnect_duration,
        TIME: data_time,
      };
    }
  } else if (port === 0x03) {
    ...
    const trigger_mode = bytes[7] & 0x01;
