      ...
      if (decodedBatch === false){
        ...
        if((cmdID === 0x0a)|(cmdID === 0x8a)|(cmdID === 0x01)){
          ...
          //simple metering
            ...
            tab.push({label: "ActiveEnergyWh", value: UintToInt(bytes[index+1]*256*256+bytes[index+2]*256+bytes[index+3],3), date: lDate});
            tab.push({label: "ReActiveEnergyVARh", value: UintToInt(bytes[index+4]*256*256+bytes[index+5]*256+bytes[index+6],3), date: lDate});
            tab.push({label: "NumberOfSample", value: (bytes[index+7]*256+bytes[index+8]), date: lDate});
            tab.push({label: "ActivePowerW", value: UintToInt(bytes[index+9]*256+bytes[index+10],2), date: lDate});
            tab.push({label: "ReActivePowerVAR", value: UintToInt(bytes[index+11]*256+bytes[index+12],2), date: lDate});
			    }
        ...
        var decoded = {};
        brData = (brUncompress(1,[{taglbl: 0,resol: 1, sampletype: 9,lblname: "ActiveEnergyWh", divide: 1}], lora.payload, lDate))
