      ...
      if (decodedBatch === false){
        ...
        if((cmdID === 0x0a)|(cmdID === 0x8a)|(cmdID === 0x01)){
          ...
          if (  (clusterdID === 0x800a) & (attributID === 0x0000)) {
            tab.push({label: "PositiveActiveEnergyWh"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+1]*256*256*256+bytes[index+2]*256*256+bytes[index+3]*256+bytes[index+4],4), date: lDate});
            tab.push({label: "NegativeActiveEnergyWh"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+5]*256*256*256+bytes[index+6]*256*256+bytes[index+7]*256+bytes[index+8],4), date: lDate});
            tab.push({label:"PositiveReactiveEnergyWh"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+9]*256*256*256+bytes[index+10]*256*256+bytes[index+11]*256+bytes[index+12],4), date: lDate});
            tab.push({label: "NegativeReactiveEnergyWh"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+13]*256*256*256+bytes[index+14]*256*256+bytes[index+15]*256+bytes[index+16],4), date: lDate});
            tab.push({label: "PositiveActivePowerW"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+17]*256*256*256+bytes[index+18]*256*256+bytes[index+19]*256+bytes[index+20],4), date: lDate});            
            tab.push({label: "NegativeActivePowerW"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+21]*256*256*256+bytes[index+22]*256*256+bytes[index+23]*256+bytes[index+24],4), date: lDate});            
            tab.push({label: "PositiveReactivePowerW"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+25]*256*256*256+bytes[index+26]*256*256+bytes[index+27]*256+bytes[index+28],4), date: lDate});
            tab.push({label: "NegativeReactivePowerW"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+29]*256*256*256+bytes[index+30]*256*256+bytes[index+31]*256+bytes[index+32],4), date: lDate});


            tab.push({label: "ActivePowerW"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+17]*256*256*256+bytes[index+18]*256*256+bytes[index+19]*256+bytes[index+20],4)- UintToInt(bytes[index+21]*256*256*256+bytes[index+22]*256*256+bytes[index+23]*256+bytes[index+24],4), date: lDate});            
            tab.push({label: "ReactivePowerW"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+25]*256*256*256+bytes[index+26]*256*256+bytes[index+27]*256+bytes[index+28],4)-UintToInt(bytes[index+29]*256*256*256+bytes[index+30]*256*256+bytes[index+31]*256+bytes[index+32],4), date: lDate});
          }
          ...
          //voltage and current metering
            ...
            tab.push({label: "Vrms"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+1]*256+bytes[index+2],2)/10, date: lDate});
            tab.push({label: "Irms"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+3]*256+bytes[index+4],2)/10, date: lDate});
            tab.push({label: "Angle"+(decoded.zclheader.endpoint+1), value: UintToInt(bytes[index+5]*256+bytes[index+6],2), date: lDate});
			  	}
          ...
          //voltage and current multi metering
            ...
            if (  (clusterdID === 0x000f ) & (attributID === 0x0402)) {
                tab.push({label: "Count", value: bytes[index]*256*256*256+bytes[index+1]*256*256+bytes[index+2]*256+bytes[index+3], date: lDate});
            };
          
            // binary input present value
            if (  (clusterdID === 0x000f ) & (attributID === 0x0055)) {
                tab.push({label: "State", value: bytes[index], date: lDate});
            };
        ...
        if(cmdID === 0x09){
          ...
          var decoded = {};
          brData = (brUncompress(1,[{taglbl: 0,resol: 1, sampletype: 10,lblname: "bEnergieActiveNegative"} ], lora.payload, lDate));
    ...
    else
    {
        ...
        if (port === 2) // SOCOMEC
        {
          console.log("ICI");
            var tab=[];
            index = 0;
            decoded.Type = bytes[index++];
            decoded.ProfilNumber = (bytes[index] & 0xf0 )>>4 ;
            decoded.ProfilVersion = (bytes[index++] & 0x0f );
            decoded.TimeStamp = bytes[index++] *255*255*255 +bytes[index++] *255*255 + bytes[index++] *255 +bytes[index++];
            console.log(decoded.ProfilNumber);
            if (decoded.ProfilNumber === 1)
            {
              tab.push({label: "PositiveActiveEnergyWh4", value: bytes[index++] *255*255*255*255*255*255*255 +bytes[index++]*255*255*255*255*255*255 + bytes[index++]*255*255*255*255*255+bytes[index++]*255*255*255*255+ bytes[index++] *255*255*255 +bytes[index++] *255*255 + bytes[index++] *255 +bytes[index++], date: lDate});
              tab.push({label: "NegativeActiveEnergyWh4", value: bytes[index++] *255*255*255*255*255*255*255 +bytes[index++]*255*255*255*255*255*255 + bytes[index++]*255*255*255*255*255+bytes[index++]*255*255*255*255+ bytes[index++] *255*255*255 +bytes[index++] *255*255 + bytes[index++] *255 +bytes[index++], date: lDate});
              tab.push({label: "PositiveReactiveEnergyWh4", value: bytes[index++] *255*255*255*255*255*255*255 +bytes[index++]*255*255*255*255*255*255 + bytes[index++]*255*255*255*255*255+bytes[index++]*255*255*255*255+ bytes[index++] *255*255*255 +bytes[index++] *255*255 + bytes[index++] *255 +bytes[index++], date: lDate});
              tab.push({label: "NegativeReactiveEnergyWh4", value: bytes[index++] *255*255*255*255*255*255*255 +bytes[index++]*255*255*255*255*255*255 + bytes[index++]*255*255*255*255*255+bytes[index++]*255*255*255*255+ bytes[index++] *255*255*255 +bytes[index++] *255*255 + bytes[index++] *255 +bytes[index++], date: lDate});
              decoded.data = tab;
            }
...
const ignore_vars = ['gateway_location','application_id','timestamp','rssi','lora_coding_rate','lora_bandwidth','lora_spreading_factor','data_rate_index','coding_rate','frequency','time','gateway_eui','snr'];
