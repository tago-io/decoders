    ...
    if (decodedBatch === false){
      ...
      if((cmdID === 0x0a)|(cmdID === 0x8a)|(cmdID === 0x01)){
        ...
        if (  (clusterdID === 0x0406 ) & (attributID === 0x0000)) {
            tab.push({label: "Occupancy", value:(bytes[index]), date: lDate});
        };
        //illuminance
        ...
        if (  (clusterdID === 0x000F ) & (attributID === 0x0055)) {
            tab.push({label: "Violation", value:(bytes[index]), date: lDate});
        };
      ...
      var decoded = {};
      // Beware: No more than seven fields can be compressed simultaneously
      // Here after, eight defined but VAQAO supports only 7 of these simultaneously
      var brData = (brUncompress(3,[
        ...
        { taglbl: 7, resol: 10, sampletype: 10,lblname: "bVOC", divide: 1} ], lora.payload, lDate))
