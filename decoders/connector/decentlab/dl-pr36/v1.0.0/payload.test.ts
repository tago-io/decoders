...
const file_path =
  "decoders/connector/decentlab/dl-pr36/v1.0.0/payload.js" as const;

  describe("Payload decoder test 1", () => {
  const payload = [
    {
      variable: "payload",
      value: "02032b0003806797810c2b",
    },
  ];
  ...
  test("Decoded values", () => {
    ...
    expect(values.device_id).toBe(811);
    expect(values.pressure).toBe(0.01);
    expect(values.protocol_version).toBe(2);
    expect(values.temperature).toBe(23.5);
  });
});
