...
const file_path =
  "decoders/connector/decentlab/dl-dlr2-008/v1.0.0/payload.js" as const;

  describe("Payload decoder test 1", () => {
  const payload = [
    {
      variable: "payload",
      value: "0207df000317de008d0c60",
    },
  ];
  ...
  test("Decoded values", () => {
    ...
    expect(values.device_id).toBe(2015);
    expect(values.protocol_version).toBe(2);
    expect(values.temperature).toBe(20.03);
  });
});
