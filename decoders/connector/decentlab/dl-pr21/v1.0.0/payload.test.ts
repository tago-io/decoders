...
const file_path =
  "decoders/connector/decentlab/dl-pr21/v1.0.0/payload.js" as const;

  describe("Payload decoder test 1", () => {
  const payload = [
    {
      variable: "payload",
      value: "02016700034e8060170c7f",
    },
  ];
  ...
  test("Decoded values", () => {
    ...
    expect(values.device_id).toBe(359);
    expect(values.pressure).toBe(0.25);
    expect(values.protocol_version).toBe(2);
    expect(values.temperature).toBe(25.67);
  });
});
