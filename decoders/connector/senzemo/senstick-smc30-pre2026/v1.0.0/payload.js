...
function batteryPercent(normalizedLevel) {
    return 62.5 * normalizedLevel - 100;
}
// Port 1 or 2: sensor data packet
function decodeDataPacket(bytes, group, time) {
    if (bytes.length < 8) {
...
function decodeConfigPacket(bytes, group, time) {
    if (bytes.length < 9) {
...
function smc30Decode(bytes, port, group, time) {
    if (port === 1 || port === 2) {
