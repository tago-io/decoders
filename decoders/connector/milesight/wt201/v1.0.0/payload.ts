function handle_downlink_response(channel_type: number, bytes: Buffer, offset: number): { data: { [key: string]: any }; offset: number } {
  ...
  switch (channel_type) {
    ...
    case 0x06: // temperature_threshold_config
      ...
      var data: { [key: string]: number | string } = { condition: condition, alarm_type: alarm_type };
    ...
    case 0x82:
      ...
      break;
    case 0x83:
      var config: { [key: string]: number | string } = {};
    ...
    case 0xb6:
      ...
      break;
    case 0xb7:
      decoded.temperature_ctl_mode = readUInt8(bytes[offset]);
    ...
    case 0xc2:
      ...
      break;
    case 0xc4:
      decoded.outside_temperature_ctl_config = decoded.outside_temperature_ctl_config || {};
      decoded.outside_temperature_ctl_config.enable = readUInt8(bytes[offset]);
      if (decoded.outside_temperature_ctl_config.enable === 1) {
        decoded.outside_temperature_ctl_config.timeout = readUInt8(bytes[offset + 1]);
      }
      offset += 2;
      break;
    case 0xc5:
      decoded.temperature_ctl_enable = readUInt8(bytes[offset]);
      offset += 1;
      break;
    case 0xc7:
      var xpto = readUInt8(bytes[offset]);
      offset += 1;

      var mask = xpto >>> 4;
      var status = xpto & 0x0f;
    ...
    case 0xc8:
      ...
      decoded.plan_config.temperature_ctl_mode = readUInt8(bytes[offset + 1]);
...
function handle_downlink_response_ext(channel_type: number, bytes: Buffer, offset: number): { data: { [key: string]: any }; offset: number } {
  var decoded: { [key: string]: any } = {};
...
function readInt8(bytes: number): number {
...
function readD2DCommand(bytes: Buffer): string {
  return ("0" + (bytes[1] & 0xff).toString(16)).slice(-2) + ("0" + (bytes[0] & 0xff).toString(16)).slice(-2);
}

function readTemperatureUnit(type: number): string {
  switch (type) {
    case 0x00:
      return "℃";
    case 0x01:
...
function readException(type: number): string {
  switch (type) {
    case 0x01:
      return "read failed";
    case 0x02:
...
function readPlanEvent(type: number): string {
...
function readPlanType(type: number): string {
...
function readSystemStatus(type: number): string {
  // 0: off, 1: on
  switch (type) {
    case 0x00:
      return "off";
...
function readWires(wire1: number, wire2: number, wire3: number): string {
  const wire: string[] = [];
  if ((wire1 >>> 0) & 0x03) {
    wire.push("Y1");
  }
  ...
  const w2_aux_wire = (wire2 >>> 6) & 0x03;
...
function readWiresRelay(status: number): { [key: string]: number | string } {
  const relay: { [key: string]: number | string } = {};
...
function readObMode(type: number): string {
  // 0: cool, 1: heat
  switch (type) {
    case 0x00:
      return "cool";
...
function readTemperatureCtlModeEnable(type: number): string {
...
function readTemperatureCtlStatusEnable(heat_mode: number, cool_mode: number): string {
...
function readWeekRecycleSettings(type: number): string {
  // bit1: "mon", bit2: "tues", bit3: "wed", bit4: "thur", bit5: "fri", bit6: "sat", bit7: "sun"
  const week_enable: string[] = [];
  if ((type >>> 1) & 0x01) {
  ...
  return week_enable.join(", ");
}

function readFanStatus(type: number): string {
...
function readFanMode(type: number): string {
...
function readTemperatureCtlStatus(type: number): string {
...
function readTemperatureCtlMode(type: number): string {
...
function mileSightDeviceDecode(bytes: Buffer): DecodedData {
  ...
  for (let i = 0; i < bytes.length; ) {
    ...
    // TEMPERATURE CONTROL
      ...
      const temperature_ctl = bytes[i];
      decoded.temperature_ctl_mode = readTemperatureCtlMode(temperature_ctl & 0x03);
      decoded.temperature_ctl_status = readTemperatureCtlStatus((temperature_ctl >>> 4) & 0x0f);
      i += 1;
    }
    // FAN CONTROL
    ...
    // PLAN
      ...
      const schedule: { [key: string]: number | string } = {};
      ...
      const time_mins = readUInt16LE(bytes.slice(i + 4, i + 6));
      schedule.time = Math.floor(time_mins / 60) + ":" + ("0" + (time_mins % 60)).slice(-2);
      i += 6;
    ...
    // PLAN SETTINGS
      ...
      const plan_setting: { [key: string]: number | string } = {};
    ...
    // HISTORICAL DATA
      ...
      const data: { [key: string]: number | string } = { timestamp: timestamp };
      ...
      const temperature_target = ((value2 >>> 5) & 0x7ff) / 10 - 100;
    ...
    // DOWNLINK RESPONSE
      ...
      const result = handle_downlink_response(channel_type, bytes, i);
      Object.assign(decoded, result.data);
      i = result.offset;
    } else if (channel_id === 0xf8) {
      const result = handle_downlink_response_ext(channel_type, bytes, i);
      Object.assign(decoded, result.data);
      i = result.offset;
    } else {
      break;
...
function parseUnit(variable: string): string {
  // 0: heat, 1: em heat, 2: cool, 3: auto
  switch (variable) {
    case "battery":
      return "%";
...
if (payload_raw) {
  try {
    ...
    let parsedTagoObj: Pick<Data, "variable" | "value" | "time" | "unit" | "group" | "metadata">[] = [];

    parsedTagoObj = Object.keys(decoded)
      .map((key) => {
        const excludeKeys = ["history", "plan_schedule", "plan_settings"];
    ...
    if (decoded.history !== undefined) {
      decoded.history.forEach((history) => {
        ...
        parsedTagoObj.push({ variable: "fan_mode", value: history.fan_mode, group, time: dateTime });
        parsedTagoObj.push({ variable: "fan_status", value: history.fan_status, group, time: dateTime });
        parsedTagoObj.push({ variable: "system_status", value: history.system_status, group, time: dateTime });
        parsedTagoObj.push({ variable: "temperature", value: history.temperature, group, unit: "°C", time: dateTime });
        parsedTagoObj.push({ variable: "temperature_ctl_mode", value: history.temperature_ctl_mode, group, time: dateTime });
        parsedTagoObj.push({ variable: "temperature_ctl_status", value: history.temperature_ctl_status, group, time: dateTime });
        parsedTagoObj.push({ variable: "temperature_target", value: history.temperature_target, group, unit: "°C", time: dateTime });
      });
    }

    if (decoded.plan_schedule !== undefined) {
      parsedTagoObj.push({
        variable: "plan_schedule",
        value: JSON.stringify(decoded.plan_schedule),
        group,
        time: new Date(time),
        metadata: { plan_schedule: decoded.plan_schedule },
      });
    }

    if (decoded.plan_settings !== undefined) {
      parsedTagoObj.push({
        variable: "plan_settings",
        value: JSON.stringify(decoded.plan_settings),
        group,
        time: new Date(time),
        metadata: { plan_settings: decoded.plan_settings },
      });
    }
