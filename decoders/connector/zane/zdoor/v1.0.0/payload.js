...
function parsePayload(payload_raw, port) {
  try {
    ...
    if (port === 207) {
      return {
        message: { value: 'Keep alive' },
        battery: { value: battery, unit: '%' },
      };
    }

    const door_event = payload_raw.substr(1, 1) === '1' ? 'Open event occured' : 'Close event occured';

    return {
      door_event: { value: door_event  },
      battery: { value: battery, unit: '%' },
    };
  } catch (e) {
    console.log(e);
...
// let payload = [{ variable: 'payload', value: '01bb' }];
