...
function parsePayload(payload_raw, port) {
  try {
    ...
    if (port === 118) {
      const brightness_level = buffer.readUInt8(0);
      const dali_status = payload_raw.substr(-1, 1) === '1' ? 'on' : 'off';

      return {
        brightness_level: { value: brightness_level, unit: '%' },
        dali_status: { value: dali_status },
      };
    }

    if (port === 202) {
      const message = payload_raw === 'DE' ? 'Device fault' : 'Device OK';
      return {
...
// let payload = [{ variable: 'payload', value: '6401' }];
