...
const file_path = "decoders/connector/mclimate/co2-display/v1.0.0/payload.js" as const;


describe("MClimate CO2 Display keepalive", () => {

  let payload = [{ variable: "payload", value: "010267820B7C9320003700" }];
  payload = decoderRun(file_path, { payload });
  const ppm = payload.find((item) => item.variable === "ppm");
...
describe("MClimate CO2 Display keepalive with command response ", () => {
  let payload = [{ variable: "payload", value: "1205010267820B7C9320003700" }];
  payload = decoderRun(file_path, { payload });
  const ppm = payload.find((item) => item.variable === "ppm");
  const sensorTemperature = payload.find((item) => item.variable === "sensorTemperature");
