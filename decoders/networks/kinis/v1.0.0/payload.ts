 ...
 ** It simples convert a raw JSON to a formatted TagoIO JSON.
 ...
 */
function toTagoFormat(object_item: any, serie: string, prefix: string = "") {
  const result: DataToSend[] = [];
...
function parseRAW(data: any, serie: string) {
  const result: DataToSend[] = [];
...
function parseDOP(data: any, serie: string) {
  const result: DataToSend[] = [];
...
function parsePRC(data: any, serie: string) {
...
if (type && data) {
  ...
  payload = JSON.parse(data.value as string);

  let vars_to_tago: DataToSend[] = [];
  ...
  // Change the payload to the new formatted variables.
