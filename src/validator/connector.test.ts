...
import { zConnector } from "./connector.ts";
...
describe("zConnector schema", () => {
  ...
  it("should fail validation for a connector object with invalid data", () => {
    const invalidData = {
      ...
      device_annotation: "Device annotation text",
      device_parameters: {
        /* add invalid data according to zIntegrationParameters */
      },
    ...
    if (!result.success) {
      ...
      expect(result.error.issues[0].message).toBe("Too small: expected array to have >=1 items");

      expect(result.error.issues[1].path[0]).toBe("device_parameters");
      expect(result.error.issues[1].message).toBe("Invalid input: expected array, received object");

      expect(result.error.issues[2].path[0]).toBe("logo");
      expect(result.error.issues[2].message).toBe("Invalid input: expected string, received number");
