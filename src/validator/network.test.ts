...
import { zNetwork } from "./network.ts";
...
describe("zNetwork schema", () => {
  ...
  it("should fail validation for a network object with invalid data", () => {
    ...
    expect(result.error?.issues[1].message).toBe('Invalid option: expected one of "lower"|"upper"|""');
    expect(result.error?.issues[2].path).toEqual(["serial_number", "label"]);
    expect(result.error?.issues[2].message).toBe("Too big: expected string to have <=100 characters");
  });
